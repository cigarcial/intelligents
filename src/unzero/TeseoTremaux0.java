...
public class TeseoTremaux0 implements AgentProgram {
	...
	public TeseoTremaux0(SimpleLanguage _language) {
		language = _language;
	}
	...
	public Action compute(Percept p) {
		if (cmd.size() == 0) {
			...
			if (MT) {
				System.out.println(" llegu� hijueputa xd");
			...
			if (primerOpcion.size() != 0) {
				...
				// implementaci�n
					...
					if (tercerOpcion.size() != 0) {
						/*
						 * int cantidad = tercerOpcion.size(); int selec =
						 * random.nextInt(cantidad); String giro =
						 * tercerOpcion.get(selec); for (int i = 0; i <
						 * selecNumGiro(giro); i++) {
